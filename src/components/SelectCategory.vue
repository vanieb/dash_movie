        ...
        <v-expansion-panels>
          <v-expansion-panel :disabled="categories.length == 0">
            <v-expansion-panel-header>
              <small
                >{{ $t("common.selected") }} {{ $t("pagination.items") }}:
                {{ selected.length }}</small
              ><br />
              <span
              ...
              <span
                ...
                class="error--text"
                >{{ $t("errors.required") }}: {{ $t("nav.category") }}
              </span>
            </v-expansion-panel-header>
            <v-row class="ma-1">
...
export default {
  ...
  methods: {
    ...
    async getFilteredCategories(categoryFilter = "", mode = "") {
      ...
      await this.$http
        .get(
          `${api.categories}?limit=${this.limit}&offset=${this.offset}&website=${this.websiteFilter}&types=${categoryFilter}&active=true`
        )
        .then((response) => {
