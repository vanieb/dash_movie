          ...
          <tbody>
            <tr v-for="item in querySet" :key="item.id">
              ...
              <td
                ...
                v-if="$root.permissions.includes('change_app_category_status')"
              >
                <v-switch
                  value
              ...
              <td>{{ item.code }}</td>
              <td>
                <span v-for="item in item.types" :key="item.id"
                  >{{ item.name }}<br
                /></span>
              </td>
              <td>
              ...
              <td
                ...
                v-if="
                  $root.permissions.includes('change_app_category_details') ||
                    $root.permissions.includes('delete_app_category')
                "
              >
                <v-btn
                  class="mr-1"
                ...
                <v-menu
                  offset-y
                  v-if="$root.permissions.includes('delete_app_category')"
                >
                  <template v-slot:activator="{ on }">
  ...
  computed: {
    ...
    cardTitle() {
      return this.isUpdate
        ...
        : `${this.$t("actions.add")} - ${this.$t("nav.category")}`;
    },
    isUpdate() {
  ...
  methods: {
    ...
    async saveCategory() {
      ...
      if (isValid) {
        ...
        if (this.category.id) {
          this.$http
            .put(`${this.categoriesApi}${this.category.id}/`, categoryResult)
            .then(
              () => {
                this.snackbar = {
                  color: "success",
