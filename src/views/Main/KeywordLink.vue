            ...
            <v-dialog v-model="showForm" persistent max-width="500">
              <template v-slot:activator="{ on }">
                <v-btn
                  ...
                  v-on="on"
                  v-if="
                    $root.permissions.includes('create_article_keywords_link')
                  "
                  align-right
                >
          ...
          <tbody>
            <tr v-for="item in querySet" :key="item.id">
              ...
              <td
                ...
                v-if="
                  $root.permissions.includes('change_article_keywords_link') ||
                    $root.permissions.includes('delete_article_keywords_link')
                "
              >
                <v-btn
                  class="mr-2"
  ...
  computed: {
    ...
    cardTitle() {
      return this.isUpdate
        ...
        : `${this.$t("actions.add")} - ${this.$t("seo.keywords")}`;
    },
    isUpdate() {
  ...
  methods: {
    ...
    async saveKeyword() {
      ...
      if (isValid) {
        if (this.keyword.id) {
          this.$http
            .put(`${this.keywordsApi}${this.keyword.id}/`, keywordResult)
            .then(
              () => {
                this.snackbar = {
                  color: "success",
