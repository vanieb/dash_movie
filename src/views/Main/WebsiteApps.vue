      ...
      <template v-slot:body="{ items }">
        ...
        <tbody>
          <tr v-for="item in querySet" :key="item.id">
            <td width="5%">
              <v-btn class="mr-2" icon color="info" :to="`/webapps/${item.id}/${query.website}/`">
                <v-icon>touch_app</v-icon>
              </v-btn>
            </td>
            <td class="align-center" width="20%">{{ item.app.name }}</td>
            <td class="align-center justify-center" width="50%">
              <span>{{item.website.name}}<br/></span>
            </td>
            ...
            <td class="align-center justify-center">
              <v-btn class="mr-2" icon :to="`/webapps/${item.id}/${query.website}/edit`">
                <v-icon>edit</v-icon>
              </v-btn>
            </td>
          </tr>
...
export default {
  ...
  methods: {
    ...
    toggle(id, value, mode){
      ...
      this.$http.put(this.webappsApi + id + '/', toggleResult).then((response) => {
