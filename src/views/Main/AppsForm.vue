          ...
          <v-container mt5>
                  ...
                  >
                    <v-card>
                      ...
                      <v-card-text>
                        ...
                        <small color="error-text"
                          >*{{ $t("errors.required") }}</small
                        >
                      </v-card-text>
                      <!-- BUTTONS -->
                      <v-card-actions>
              ...
              <tbody>
                ...
                >
                  ...
                  <td v-if="classification.labels">
                    <span
                      v-for="label in classification.labels"
                      :key="label.id"
                    >
                  ...
                  <td>
                    <v-icon
                      small
                      @click="editClass(classification)"
                      class="mr-2"
                      >edit</v-icon
                    >&nbsp;
                    <v-icon
              ...
              <v-card-text>
                ...
                <v-row>
                  <v-col>
                    <validation-provider
                      style="width:338px;"
                      :rules="`${apps.use_android_link ? 'required' : ''}`"
                      :name="`${$t('apps.external_download_link')}(Android)`"
                    >
                      <v-text-field
            ...
            <v-layout justify-start mt-3>
              <v-btn
                ...
                @click="saveApp(apps.status ? apps.status : 'draft')"
              >
...
export default {
  ...
  computed: {
    cardTitle() {
      return this.isUpdateClass
        ? `${this.$t("actions.update")} - ${this.$t("apps.classification")} `
        : `${this.$t("actions.add")} - ${this.$t("apps.classification")}`;
    },
    cardIcon() {
  ...
  methods: {
    async getAppDetails(id) {
      this.loading = true;
      this.id = id;
      await this.$http.get(`${this.appsApi}${id}/`).then(
        (response) => {
          this.apps = response;
    ...
    async getAppClassDetails(id) {
      ...
      await this.$http.get(`${this.classApi}/${id}/`).then(
        (response) => {
    ...
    typeSelectOne(val) {
      ...
      this.websiteFilter = this.isUpdate
        ? this.apps.website.id
        : this.apps.website;
    ...
    async saveClass() {
      ...
      if (
        !this.app_classification.labels ||
        !this.app_classification.categories
      ) {
        this.snackbar = {
          color: "red",
          show: true,
          text: `${this.$t("nav.labels")}/${this.$t(
            "apps.category"
      ...
      if (isValid) {
        ...
        await this.$http
          .get(`${api.types}?limit=400&offset=0&${this.typeFilter}`)
          .then((response) => {
            this.types = response.results;
          });
        this.class_type = this.types.filter(
          (element) => element.code == this.app_classification.type
        );
        ...
        this.$http.put(`${this.classApi}/${this.apps.slug}/`, formData).then(
          (response) => {
            ...
            let snackbarText = this.isUpdateClass
              ? this.$t("actions.update")
              : this.$t("actions.add");
            this.snackbar = {
              color: "success",
              show: true,
              text: `${snackbarText} ${this.$t(
                "apps.classification"
    ...
    deleteClass(id) {
      ...
      this.$http
        .delete(`${this.appsApi}types/${this.id}/`, { data: typeIdForm })
        .then(
          () => {
            this.snackbar = {
              color: "success",
              show: true,
              text: `${this.$t("actions.delete")} ${this.$t(
                "apps.classification"
              )}: ${this.$t("status.success")}`,
            };
    ...
    async saveApp(status) {
      ...
      if (isValid) {
        ...
        if (this.isUpdate) {
          ...
          this.$http.post(this.appsApi, formData).then(
            (response) => {
              this.snackbar = {
                color: "success",
                show: true,
